...
| **7** | **Tích hợp 4 Official Capacitor Plugins** | ✅ Complete | Triển khai đầy đủ 4 official plugins theo đúng yêu cầu Slide Tuần 5: (1) `@capacitor/camera` chụp ảnh chất lượng cao native kèm fallback web, (2) `@capacitor/geolocation` định vị GPS hiện trường chính xác thay thế `navigator.geolocation`, (3) `@capacitor/filesystem` lưu ảnh chụp vào bộ nhớ file hệ thống cục bộ trước khi đồng bộ, (4) `@capacitor/push-notifications` & `@capacitor/local-notifications` kích hoạt thông báo hệ thống Native khi đồng bộ hoàn tất (sync-success alerts). |
| **8** | **Đóng gói & Biên dịch Android Native APK** | ✅ Complete | Đã thiết lập chuẩn cấu hình `capacitor.config.ts`, cập nhật đầy đủ quyền phần cứng (`CAMERA`, `ACCESS_FINE_LOCATION`, `ACCESS_COARSE_LOCATION`, `POST_NOTIFICATIONS`, `VIBRATE`) trong `AndroidManifest.xml` với App ID `com.vku.fieldsurvey`. Biên dịch thành công tệp `app-debug.apk` sẵn sàng cài đặt trực tiếp trên Android. |
...
| **7** | **Định vị GPS Hiện trường (Capacitor Geolocation)** | Native / Web | Nút *"Lấy tọa độ GPS hiện trường"* gọi `@capacitor/geolocation`, trả về chính xác Latitude, Longitude (±5m), nhúng liên kết Google Maps và tự động lưu vào phiếu khảo sát. | **ĐẠT** |
| **8** | **Lưu trữ ảnh vào bộ nhớ tệp (Filesystem)** | Native / Disk | Plugin `@capacitor/filesystem` ghi ảnh chụp trực tiếp vào phân vùng đĩa cục bộ (`Directory.Data/survey_photos/`) trước khi đồng bộ, giải phóng RAM và tránh phình to IndexedDB. | **ĐẠT** |
| **9** | **Thông báo đẩy khi đồng bộ thành công (Sync Alerts)** | Native Android | Sau khi kết nối mạng được phục hồi và `SyncManager` đẩy phiếu thành công, `@capacitor/local-notifications` và `push-notifications` phát thông báo Native kèm rung và âm thanh: *"Đồng bộ dữ liệu thành công! ✅"*. | **ĐẠT** |
| **10** | **Đóng gói và Biên dịch APK Android** | Android Native | Sử dụng Gradle Wrapper biên dịch thành công tệp `app-debug.apk` (10.15 MB, 280 tasks) với package `com.vku.fieldsurvey`, sẵn sàng cài đặt trực tiếp lên điện thoại Android thực tế. | **ĐẠT** |
| **11** | **Đường ống Phân tích Dữ liệu Python** | CLI | Script `analysis.py` tự động đọc CSV xuất từ Google Sheets, xử lý khuyết thiếu và tạo thành công 6 biểu đồ phân tích tương quan tại `/analysis/output/`. | **ĐẠT** |
...
### Thách thức 3: Quản lý tài nguyên phần cứng & phản hồi trải nghiệm Native khi di chuyển từ PWA
* **Vấn đề (Bottleneck):** Khi chạy dưới dạng Web/PWA, dung lượng lưu trữ ảnh Base64 trong IndexedDB dễ bị trình duyệt dọn dẹp (eviction) nếu dung lượng lớn, đồng thời người dùng không nhận được phản hồi trực quan khi ứng dụng hoàn thành đồng bộ trong nền.
* **Giải pháp (Resolution):**
  1. Sử dụng `@capacitor/filesystem` để lưu trữ ảnh chụp vật lý vào thư mục an toàn của hệ điều hành Android (`Directory.Data`), chỉ lưu đường dẫn file cục bộ trong cơ sở dữ liệu thay vì nhồi nhét chuỗi Base64 dài hàng triệu ký tự.
  2. Kết hợp `@capacitor/local-notifications` và `@capacitor/push-notifications`: Khi `SyncManager` hoàn thành gửi các phiếu trong hàng đợi, hệ thống tạo kênh thông báo Android độ ưu tiên cao (`importance: 4`), rung chuông và hiển thị banner thông báo tới thanh trạng thái của hệ điều hành, mang lại trải nghiệm 100% như ứng dụng Android thuần túy (Native App).
